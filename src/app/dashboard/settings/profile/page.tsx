...
export default function ProfilePage() {
  const { hydrated } = useAuthStore(); // wait for auth hydration
  const { user, loading, updateUser } = useProfile();
  ...
  const handleSave = async () => {
    ...
    setSaving(true);
    try {
      await updateUser({ name: form.name, username: form.username });
  ...
  // Show loading until auth state is hydrated
